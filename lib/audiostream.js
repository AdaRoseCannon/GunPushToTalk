...
import { process, toWav } from "./pcm.js";
...
export class AudioStream {
	...
	async getRecorder() {
		...
		mediaRecorder.addEventListener("dataavailable", async (event) => {
			...
			console.log("Decoding audio data from the recorder", buffer);

			// MediaRecorder does not support PCM out of the box -> output needs to be decoded into PCM
			context.decodeAudioData(buffer, (audioBuffer) => {
				// On first chunk of data send the
				if (startedRecording) {
					// sends meta-data before the first chunk
					const meta = {
				...
				const data = process(audioBuffer);
	...
	getNewPlayer(metadata) {
		...
		function play(data) {
			...
			const wavBuffer = toWav(opts, data);

			console.log("Decoding audio data from play");
